...
@section('content')
    <div class="container">
        <div class="d-flex justify-content-end mt-5">       
            <a href="#print" class="btn btn-blue btn-sm white-text printer"><span class="icofont icofont-print"></span> imprimer</a>
        </div>
        <div class="row mt-3">
                        ...
                        <tbody>
                            @foreach ($messages as $message)
                                <?php
                                    ...
                                    $montant = (30 * $nbre_pages) * $message->destinataires;

                                    $montant_total += $montant;
                                ?>
                                <tr>
                                    <td>{{ \Illuminate\Support\Str::substr($message->body, 0, 25) . '...' }}</td>
                                    <td>{{ \Illuminate\Support\Carbon::parse($message->created_at)->format('d-m-Y') }}</td>
                                    <td>{{ $nbre_pages }}</td>
                                    <td>{{ $message->destinataires }}</td>    
                                    <td>{{ $montant }}</td>    
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="d-flex justify-content-center">
            <a href="{{ route('structure.factureIndex') }}" class="btn btn-grey white-text">retour</a>
        </div>
    </div>
@endsection
